...
func TestResolveClientIPXFFRightmostInTrust(t *testing.T) {
	r, _ := http.NewRequest("GET", "/", nil)
	r.RemoteAddr = "10.0.0.5:1"
	// XFF: rightmost is the most-recent (trusted) proxy hop. Walking
	// right-to-left, skip trusted IPs, return first untrusted.
...
func TestResolveClientIPNoTrustedHeaders(t *testing.T) {
	r, _ := http.NewRequest("GET", "/", nil)
	r.RemoteAddr = "10.0.0.5:1"
	trusted := []*net.IPNet{mustCIDR("10.0.0.0/8")}
	// No headers list, so even though peer is trusted, return peer.
	ip := ResolveClientIP(r, trusted, nil)
