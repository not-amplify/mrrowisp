...
// For returns the detector for the (connID, dst) tuple, creating it on
// first use. Returns a nop detector when signatures are disabled.
...
// Forget evicts every detector that belongs to connID.
...
// Record stores a single dial outcome (ok=true means handshake completed).
// On a nop detector this is a no-op.
...
// Match reports whether the current ring satisfies the SYN-flood
// signature: at least MinSamples observations and at least
// FailedHandshakeRatio of them failed.
